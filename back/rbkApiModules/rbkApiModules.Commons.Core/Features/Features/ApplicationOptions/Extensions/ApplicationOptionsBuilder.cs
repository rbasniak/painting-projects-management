...
public static class ApplicationOptionsBuilder
{
    ...
    public static IApplicationBuilder UseApplicationOptions(this IApplicationBuilder app)
    {
        ...
        using (var scope = scopeFactory.CreateScope())
        {
            ...
            // Load or create options for every registered IApplicationOptions singleton
            var optionsObjects = scope.ServiceProvider.GetServices<IApplicationOptions>();

            if (optionsObjects != null)
            {
                foreach (var options in optionsObjects)
                {
    ...
    private static void PopulateOptionsFromDatabase(DbContext dbContext, object optionsRoot)
    {
        ...
        TraverseAndApply(dbContext, optionsRoot.GetType().Name, optionsRoot, parentKey: null);
    }
    ...
    private static void TraverseAndApply(DbContext context, string rootName, object currentObject, string parentKey)
    {
        ...
        foreach (var property in type.GetProperties(BindingFlags.Instance | BindingFlags.Public))
        {
            ...
            {
                // Ensure exists in DB
                var option = set.AsNoTracking().SingleOrDefault(x => x.Key == key);
                if (option == null)
                {
                    var stringValue = defaultValue != null ? ConvertToString(defaultValue, propertyType) : string.Empty;
                    
                    option = new ApplicationOption(key, stringValue);
                ...
                {
                    var typedValue = ConvertFromString(option.Value, propertyType);
